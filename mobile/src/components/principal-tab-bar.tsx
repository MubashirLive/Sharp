...
const PRINCIPAL_TAB_CONFIG: Record<string, { Icon: LucideIcon; label: string }> = {
  chat:          { Icon: MessageCircle, label: 'Chat' },
  attendance:    { Icon: BarChart3,     label: 'Reports' },
  index:         { Icon: Building2,     label: 'Dashboard' },
  announcements: { Icon: Megaphone,     label: 'Notices' },
  staff:         { Icon: BadgeCheck,    label: 'Staff' },
};
...
function TabButton({
  ...
  const config = PRINCIPAL_TAB_CONFIG[routeName];
...
export default function PrincipalTabBar({
  state,
  descriptors,
  navigation,
}: any) {
  ...
  return (
    ...
    >
      ...
      {state.routes.map((route: any, index: number) => {
        const config = PRINCIPAL_TAB_CONFIG[route.name];
        if (!config) return null;
