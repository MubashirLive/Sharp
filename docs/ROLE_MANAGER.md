...
**Removed in 2026-06-18 rebuild.** Per-dept Settings (gear, messenger JSON editor) + Log (history, `DepartmentLogPanel`). Both surfaces now live exclusively in My School per the [MY_SCHOOL.md](MY_SCHOOL.md) Tab 6 ownership split:
...
| Tab | Status |
|-----|--------|
| Staff | Complete. See §3.1 (9-section drawer, auto-derived Role, self-assign guard). |
| Subjects | Complete. See §3.2 (section-block grid, per-cell save, busy-key race guard). |
| Wings | Complete. Spec locked 2026-06-06; see §3.3. Uses `wing_staff`; shared cache (§4.5). |
| Departments | Complete. Per-card edit/save, draft flow, shared `useSaveDepartmentAssignments`. Settings + Log surfaces owned by My School (see §3.4). |
| Houses | Complete. Per-card edit/save, draft flow, `HouseMovePromptDialog` move gate (§3.5). |

> Patch-level history (bug fixes, migrations, per-session change logs) lives in git and `docs/LESSONS.md`. This doc describes current behavior only.
