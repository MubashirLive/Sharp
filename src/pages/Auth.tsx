...
export default function AuthPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!signedInUserId) {
      setBusy(false);
      toast({ title: "Sign in failed", description: "Unable to load signed-in user.", variant: "destructive" });
    ...
    if (profileError || !profile) {
      toast({ title: "Profile not found", description: profileError?.message ?? "This account does not have an app profile yet.", variant: "destructive" });
      await supabase.auth.signOut();
      return;
    }
  ...
  return (
    <LoginCard
      brandingIcon={<GraduationCap className="h-8 w-8" />}
      brandingLabel="SHARP"
      title="Welcome back"
      subtitle="Sign in to continue to your school dashboard"
      footer={
        <>
          No account yet?{" "}
          <span className="text-[hsl(var(--login-text-muted))]">
    ...
    >
      <form onSubmit={handleLogin} className="space-y-4">
        <div className="space-y-1.5">
          <Label htmlFor="li-email" className="text-sm font-medium text-[hsl(var(--login-text))]">
            Email address
          </Label>
          <Input
            id="li-email"
        ...
        <div className="space-y-1.5">
          <Label htmlFor="li-pwd" className="text-sm font-medium text-[hsl(var(--login-text))]">
            Password
          </Label>
